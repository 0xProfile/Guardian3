import { Box, Card, CardContent, Checkbox, List, ListItem, ListItemAvatar, ListItemButton, ListItemText } from "@mui/material";
...
export default function selectReport({ checked, setChecked }) {
    ...
    useEffect(() => {
        if (data && signer) {
            ...
            address.then((addr) => {
                setReporters(data.filter((it) => it[4] != addr).map((it) => {
                    return {
                        name: it[0],
                        email: it[1],
                        org: it[2],
                                ...
                                <ListItem key={index} 
                                    ...
                                    <ListItemButton>
                                        <Box sx={{
                                            ...
                                            <ListItemText secondary={`Email: ${reporter.email}`} primary={`Name: ${reporter.name}`} />
